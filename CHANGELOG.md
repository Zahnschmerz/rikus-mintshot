...
**🇩🇪 Verlässliche Fortschritts- und Zeitanzeige beim Bauen · Secure-Boot auch bei knappem Platz · Name als Eigenprodukt aufgeräumt.**

- **Neu: der Secure-Boot-Umbau macht sich den Platz jetzt selbst.** Damit die ISO auch mit eingeschaltetem Secure Boot startet, baut das Programm sie am Ende neu zusammen. Dafür lagen bisher kurz **zwei** ISOs gleichzeitig auf der Platte — bei einer großen ISO („System + Home") wurde der Platz leicht knapp, und man bekam nur die Meldung „mach erst X MB frei und bau neu". Jetzt erkennt das Programm: Der komplette Inhalt liegt ohnehin schon im Arbeitsordner, die fertige ISO ist nur eine Kopie davon. Reicht der Platz nicht für eine zweite, **ersetzt** es die fertige ISO direkt aus dem Arbeitsordner — so wird auch eine riesige ISO Secure-Boot-fähig, ohne dass du von Hand Platz schaffen musst. Ist genug Platz da, bleibt es beim sicheren Weg (die fertige ISO wird erst ganz am Schluss ersetzt). Und geht bei extrem knappem Platz doch etwas schief, sagt das Programm **klar**, was zu tun ist — kein stiller Ausfall.
- **Behoben: die Prozentanzeige sprang beim Bauen manchmal zu früh auf 95 %.** Im Modus „System + Home" schreibt das Bau-Werkzeug **jeden kopierten Dateinamen** mit. Hieß eine Datei zufällig wie ein Bau-Schritt (z. B. eine Notiz mit „Secure-Boot" im Namen), verwechselte die Anzeige den Dateinamen mit dem Schritt und sprang auf 95 %, während in Wirklichkeit noch komprimiert wurde. **Der Bau selbst war nie betroffen — nur die Anzeige.** Jetzt zählen nur noch echte Fortschritts-Meldungen, keine Dateinamen mehr.
- **Neu: eine Zeitanzeige beim Bauen.** Neben dem Schritt läuft jetzt eine Uhr mit (⏱ Minuten:Sekunden), und am Ende steht, wie lange der ganze Bau gedauert hat — so sieht man auf einen Blick, dass es vorangeht.
- **Behoben: der Live-Stick konnte beim Start hängen bleiben (kein Schreibtisch).** Der Dienst, der nach dem Wiederherstellen die SSH-Schlüssel sicherstellt, startete im **Live-Betrieb zu früh** — noch bevor Mints eigenes `live-config` die Schlüssel anlegt. Dadurch legte er sie selbst an und startete den SSH-Dienst neu, der aber aufs Netzwerk wartete, das wiederum auf ihn wartete: Der Start blieb stehen, der Schreibtisch kam nie. Jetzt läuft der Dienst normal eingereiht (nach `live-config`), und der SSH-Neustart wartet nicht mehr — der Live-Stick startet sauber bis zum Schreibtisch durch. Auf fest installierten Klonen ändert sich nichts.
- **Namen aufgeräumt: Rikus Mintshot tritt jetzt als das auf, was es ist — ein eigenständiges Programm.** In der Anzeige, in dieser Liste und auf der Webseite wird kein fremder Programmname mehr als „Motor" genannt. Der Großteil ist eigene Technik: die Secure-Boot-Kette, die Anpassung des Klons an fremde Hardware, die Persistenz auf MBR- und GPT-Sticks, die ehrliche Platzvorschau und ein eigens eingerichteter Installer. Am eigentlichen Bau ändert sich nichts.

**🇬🇧 Reliable progress and time display while building · Secure Boot even on tight space · naming cleaned up as a standalone product.**

- **New: the Secure-Boot rebuild now frees its own space.** To make the ISO start with Secure Boot enabled, the program rebuilds it at the end. That briefly kept **two** ISOs on disk at once — on a large ISO ("System + Home") space could run short, and you only got a "free X MB and rebuild" message. Now the program knows the full content is already in the work folder and the finished ISO is just a copy of it. If there isn't room for a second one, it **replaces** the finished ISO straight from the work folder — so even a huge ISO becomes Secure-Boot-capable without you clearing space by hand. When there is enough room, it keeps the safe route (the finished ISO is only replaced at the very end). And if something still fails on extremely tight space, the program says **clearly** what to do — no silent failure.
- **Fixed: the progress bar sometimes jumped to 95% too early.** In "System + Home" mode the build tool logs **every copied file name**. If a file happened to be named like a build step (e.g. a note with "Secure-Boot" in its name), the display mistook the file name for the step and jumped to 95% while compression was still running. **The build itself was never affected — only the display.** Now only real progress messages count, no file names.
- **New: a time display while building.** A clock now runs next to the step (⏱ minutes:seconds), and at the end it shows how long the whole build took — so you can see at a glance that it's making progress.
- **Fixed: the live stick could hang on boot (no desktop).** The service that ensures SSH host keys after restoring started **too early in live mode** — before Mint's own `live-config` creates the keys. So it created them itself and restarted the SSH service, which was waiting for the network, which was waiting for it: the boot stalled and the desktop never appeared. The service is now ordered normally (after `live-config`) and the SSH restart no longer waits — the live stick boots cleanly to the desktop. Nothing changes on permanently installed clones.
- **Naming tidied up: Rikus Mintshot now presents itself as what it is — a standalone program.** In the app, in this list and on the website, no external program name is credited as the "engine" any more. Most of it is our own work: the Secure Boot chain, adapting the clone to foreign hardware, persistence on MBR and GPT sticks, the honest space preview and a custom-configured installer. The actual build is unchanged.
...
- **Added: `wget` is now a requirement.** The first-time setup uses it to download the base ISO-build helper, but none of the previous requirements pulled it in. Linux Mint ships it by default, which is why it never showed up here — on a slim Debian the setup aborted before it started.
